...
def clean_duplicate_json_keys(obj):
    """
    Recursively clean duplicate keys in JSON-like objects.

    This handles cases where LLM responses contain duplicate keys in their
    JSON output (e.g., "context":"...", "context":"...").
    When duplicate keys exist, only the first occurrence is kept.
    """
    if isinstance(obj, dict):
        # Process dict - keep only first occurrence of duplicate keys
        cleaned = {}
        for key, value in obj.items():
            if key not in cleaned:
                cleaned[key] = clean_duplicate_json_keys(value)
        return cleaned
    elif isinstance(obj, list):
        # Recursively clean list items
        return [clean_duplicate_json_keys(item) for item in obj]
    elif isinstance(obj, str):
        # Check if string contains JSON (common in LLM responses)
        # Try to parse and clean nested JSON
        try:
            if obj.strip().startswith('{') or obj.strip().startswith('['):
                parsed = json.loads(obj)
                cleaned = clean_duplicate_json_keys(parsed)
...
def save_results_to_json(results_data, filename):
    ...
    Also cleans any duplicate JSON keys that may have been generated by LLMs.
    """
    filepath = Path(filename)
    # Use Path.parent and Path.mkdir() for robust directory creation
    filepath.parent.mkdir(parents=True, exist_ok=True)

    # Clean duplicate keys before saving
    cleaned_data = clean_duplicate_json_keys(results_data)
